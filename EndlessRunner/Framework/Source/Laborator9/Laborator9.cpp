...
void Laborator9::Init()
{
	...
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "texture-can-ground-0010-1200.png").c_str(), GL_REPEAT);
...
void Laborator9::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		float x = GetSceneCamera()->transform->GetWorldPosition().x;
		float z = GetSceneCamera()->transform->GetWorldPosition().z;

		modelMatrix = glm::rotate(modelMatrix, atan2(x, z), glm::vec3(0, 1, 0));
...
void Laborator9::RenderSimpleMesh(int f_pamant, Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	// Bind pamant_test
	int loc_pamant = glGetUniformLocation(shader->program, "f_pamant");
	glUniform1i(loc_pamant, f_pamant);
